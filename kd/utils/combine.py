...
class ChannelCombine(nn.Module):
    def __init__(self, num_hiddens, combine_type, num_channels=None, attn_drop=0):
        super().__init__()
        ...
        if combine_type in ['mean']:
            ...
            self.attention = nn.Sequential(
                nn.Linear(num_hiddens, num_hiddens), nn.Tanh(), nn.Linear(num_hiddens, 1, bias=False), nn.Dropout(self.attn_drop)
            )
            ...
            for _ in range(self.num_heads):
                self.attentions.append(nn.Sequential(
                    nn.Linear(num_hiddens, num_hiddens), nn.ReLU(), nn.Dropout(p=0.2), nn.Linear(num_hiddens, 1, bias=False), nn.Dropout(self.attn_drop)
                ))
        elif combine_type == 'jk':
            self.transformation = nn.Linear(num_hiddens * num_channels, num_hiddens)
            self.global_attention = nn.Sequential(
                nn.Linear(num_hiddens*2, num_hiddens), nn.Tanh(), nn.Linear(num_hiddens, 1, bias=False), nn.Dropout(self.attn_drop)
            )
