...
def train_student(cfg, dataset, n_final_repeats):
    expt = Experiment(cfg, dataset=dataset, n_runs=n_final_repeats)
    expt.run()


def adjust_cfg(t_cfg, s_cfg):
    if t_cfg.trainer.ckpt_dir != s_cfg.trainer.kd.knowledge_dir:
...
def main(t_cfg, s_cfg, args):
    ...
    train_student(s_cfg, dataset, args.n_final_repeats)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tuner_cfg', type=str, default='./examples/KDMLP_tuner.py')
    ...
    parser.add_argument('--n_final_repeats', type=int, default=1)
